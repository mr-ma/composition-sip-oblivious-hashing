...
Slicer::Slicer(llvm::Module *M)
    : m_slice_id(0), m_module(M), m_PTA(new dg::LLVMPointerAnalysis(m_module)),
      m_RD(new dg::analysis::rd::LLVMReachingDefinitions(m_module, m_PTA.get())), m_dg(new dg::LLVMDependenceGraph()) {
  buildDG();
  computeEdges();
}
