...
private:
  ...
  std::pair<llvm::CallInst *, llvm::ZExtInst *> getValueHashingCall(llvm::Instruction *start_from,
                                                                    llvm::Value *hashedvalue);
...
FunctionExtractionHelper::FunctionExtractionHelper(llvm::Function *F,
                                                   ...
                                                   const llvm::MDNode *assert_md,
                                                   OHStats &stats)
    : m_F(F), m_path(oh_path), m_inputDepRes(inputDepRes), m_loopInfo(loop_info), m_domTree(domTree),
      m_skippedInstrs(skipped_instrs), m_argReachableInstrs(arg_reachable_instrs),
      m_globalReachableInstrs(global_reachable_instrs), m_assert(assert), m_hashFunc1(hashFunc1),
      m_hashFunc2(hashFunc2), m_assertMd(assert_md), m_stats(stats), m_pathF(nullptr) {
}
...
std::pair<llvm::CallInst *,
          llvm::ZExtInst *> FunctionExtractionHelper::getValueHashingCall(llvm::Instruction *start_from,
                                                                          llvm::Value *hashedvalue) {
...
bool ObliviousHashInsertionPass::isInstAGuard(llvm::Instruction &I) {
  if (I.getMetadata(GUARD_META_DATA)) {
    return true;
  }
  return false;
}
...
bool ObliviousHashInsertionPass::instrumentInst(llvm::Instruction &I,
  ...
  if (hashInserted) {
    ...
    if (isCallGuard) {
      ...
      is_local_hash ? stats.addNumberOfShortRangeProtectedArguments(protectedArguments) : stats
          .addNumberOfProtectedArguments(protectedArguments);
    }
  } else {
...
void ObliviousHashInsertionPass::doInsertAssert(llvm::Instruction &instr,
  ...
  long placeholder = (assertCnt) * 1000000000000;
  ConstantInt *const_int = (ConstantInt *) ConstantInt::get(
      Type::getInt64Ty(Ctx), APInt(64, placeholder));
  ...
  assert(BB != nullptr);
  auto
      *m = new Manifest(name, BB, BB, patchFunction, constraints, true, undoValues, std::to_string(placeholder) + "\n");
...
void ObliviousHashInsertionPass::setup_functions() {
  ...
  llvm::FunctionType *function_type = llvm::FunctionType::get(llvm::Type::getVoidTy(Ctx), params, false);
  hashFunc1 = llvm::dyn_cast<llvm::Function>(m_M->getOrInsertFunction(ObliviousHashInsertionPass::oh_hash1_name,
                                                                      function_type));
  hashFunc2 = llvm::dyn_cast<llvm::Function>(m_M->getOrInsertFunction(ObliviousHashInsertionPass::oh_hash2_name,
                                                                      function_type));
...
ObliviousHashInsertionPass::insert_hash_variable(llvm::Function *F,
  ...
  auto local_store =
      new llvm::StoreInst(llvm::ConstantInt::get(llvm::Type::getInt64Ty(path_Ctx), 0), local_hash, false, 8);
...
bool ObliviousHashInsertionPass::runOnModule(llvm::Module &M) {
  ...
  m_M = &M;
  if (shortRangeOH) {
    m_slicer = std::make_unique<Slicer>(m_M);
  }
