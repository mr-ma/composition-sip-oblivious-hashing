...
bool OHRunner::skipFunction(llvm::Function &F) const {
  ...
  if (!m_function_filter_info->get_functions().empty() &&
      !m_function_filter_info->is_function(&F)) {
...
bool OHRunner::skipInstruction(llvm::Instruction &I) const {
  ...
  if (auto callInst = llvm::dyn_cast<llvm::CallInst>(&I)) {
    ...
    if (calledF &&
        (calledF->getName() == "assert" || calledF->getName() == "hash1" ||
...
bool OHRunner::skipBasicBlock(llvm::BasicBlock &B) const {
  auto F_input_dependency_info =
      m_input_dependency_info->getAnalysisInfo(B.getParent());
  if (!F_input_dependency_info ||
      F_input_dependency_info->isInputDependentBlock(&B))
    ;
}
