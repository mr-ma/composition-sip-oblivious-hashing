...
bool FunctionCallSiteInformationPass::runOnModule(llvm::Module &M) {
  ...
  const auto
      &indirectCallAnalysis = getAnalysis<input_dependency::IndirectCallSitesAnalysis>().getIndirectsAnalysisResult();
  const auto
      &virtualCallAnalysis = getAnalysis<input_dependency::IndirectCallSitesAnalysis>().getVirtualsAnalysisResult();
  llvm::CallGraph *callGraph = &getAnalysis<llvm::CallGraphWrapperPass>().getCallGraph();
  const auto
      &loopInfoGetter = [this](llvm::Function &F) { return &getAnalysis<llvm::LoopInfoWrapperPass>(F).getLoopInfo(); };
