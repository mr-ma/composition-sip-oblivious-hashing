...
void MemoryDefinitionData::collectDefiningData() {
  ...
  for (auto &B : m_F) {
    for (auto &I : B) {
      ...
      auto *use = llvm::dyn_cast<llvm::MemoryUse>(useOrDef);
      if (!use) {
        continue;
      }
      ...
      if (auto *phiAccess = llvm::dyn_cast<llvm::MemoryPhi>(definingAccess)) {
      ...
      } else if (auto *defAccess = llvm::dyn_cast<llvm::MemoryDef>(definingAccess)) {
