...
public:
  OHPathsFunction(llvm::Function *F, const oh::FunctionOHPaths &paths) : m_F(F) {
...
public:
  ...
  static std::string getEdgeSourceLabel(const BasicBlock *Node,
    ...
    if (const auto *BI = dyn_cast<BranchInst>(Node->getTerminator()))
      if (BI->isConditional())
        return (I == succ_begin(Node)) ? "T" : "F";

    // Label source of switch edges with the associated value.
    if (const auto *SI = dyn_cast<SwitchInst>(Node->getTerminator())) {
...
FunctionOHPaths::FunctionOHPaths(llvm::Function *F,
    ...
    : m_F(F), m_domTree(domTree), m_loopInfo(loopInfo) {
}
